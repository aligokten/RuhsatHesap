...
| Foseptik/trafo gibi yapılar inşaat alanına eklenmiyor | `TIP=EK_YAPI` yerine tanınmayan başka bir TIP veya `BLOK=`/`KAT=` ile etiketlenmiş; `EK_YAPI` blok/kat gerektirmez, `AD=` ile adlandırılır. Etiketlemeden elle girmek isterseniz RHPANEL'deki **Ek Yapılar** sekmesini kullanın — orada blok/kat sütunu yoktur |
| RHPANEL'de girdiğim satır kaydedilmiyor / kayboluyor | Katlar ve Bağımsız Bölümler sekmelerinde `Blok` zorunludur; boş bırakılan satır atlanır (kaydettiğinizde kaç satırın atlandığı yazılır). Bloka ait olmayan yapılar **Ek Yapılar** sekmesine girilir |
| Hesaplanan alan (ör. %30 toplamı) gerçek alanın ~2 katı çıkıyor | Aynı bölge hem `HATCH` (taralı dolgu) hem sınır çizgisiyle (polyline/region) etiketlenmiş; `RHTARA` bu durumda iki nesnenin handle'ını adı geçiren **ÇİFT ETİKET** uyarısı verir (uyarılar RHPANEL'in altında da listelenir) — uyarıdaki nesnelerden yalnızca birinde etiket bırakın. İki alan birebir aynı okunmaz (ör. 4,69 + 4,68 = 9,37), bu yüzden toplam tam iki katı görünmeyebilir |
